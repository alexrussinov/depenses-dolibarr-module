...
require(DOL_DOCUMENT_ROOT.'/factures_depenses/class/paiementdepenses.class.php');
require(DOL_DOCUMENT_ROOT.'/factures_depenses/class/depenses.class.php');
...
if ($_POST['action'] == 'confirm_delete' && $_POST['confirm'] == 'yes' && $user->rights->depenses->d->supprimer)
{
	$db->begin();
	...
	{
		$db->commit();
		Header('Location: '.DOL_URL_ROOT.'/factures_depenses/paymentdep.php');
		exit;
	}
	else
	{
...
if ($result > 0)
{
	...
	*/
	...
	if (empty($_GET['action']) || $_GET['action']!='edit_date')
	{
		...
		print '</td>';
		if ($paiement->statut == 0 && $_GET['action'] != 'edit_date') print '<td align="right"><a href="'.DOL_URL_ROOT.'/factures_depenses/paiement/fiche.php?id='.$paiement->id.'&action=edit_date">'.img_edit($langs->trans('Modify')).'</a></td>';
		print '</tr></table>';
		print '</td>';
	...
	else
	{
		...
		print '<td colspan="3">';
		print '<form name="formsoc" method="post" action="'.DOL_URL_ROOT.'/factures_depenses/paiement/fiche.php?id='.$paiement->id.'"><input type="hidden" name="action" value="update_date" />';
	...
	if (empty($_GET['action']) || $_GET['action'] != 'edit_num')
	{
		...
		print '</td>';
		if ($paiement->statut == 0 && $_GET['action'] != 'edit_num') print '<td align="right"><a href="'.DOL_URL_ROOT.'/factures_depenses/paiement/fiche.php?id='.$paiement->id.'&action=edit_num">'.img_edit($langs->trans('Modify')).'</a></td>';
		print '</tr></table>';
		print '</td>';
	...
	else
	{
		...
		print '<td colspan="3">';
		print '<form name="formsoc" method="post" action="'.DOL_URL_ROOT.'/factures_depenses/paiement/fiche.php?id='.$paiement->id.'"><input type="hidden" name="action" value="update_num" />';
		print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
	 ...
	 */
	$allow_delete = 1 ;
	$sql = 'SELECT f.rowid as ref, f.ref as ref_supplier, f.total_ttc, pf.amount, f.rowid as facid, f.paye, f.fk_statut, s.nom, s.rowid as socid';
	$sql .= ' FROM '.MAIN_DB_PREFIX.'paiementdepenses_facturedep as pf,'.MAIN_DB_PREFIX.'depenses as f,'.MAIN_DB_PREFIX.'societe as s';
	...
	if ($resql)
	{
		...
		if ($num > 0)
		{
			...
			while ($i < $num)
			{
				...
				print '<tr '.$bc[$var].'>';
				// Ref
				print '<td><a href="'.DOL_URL_ROOT.'/factures_depenses/fiche.php?id='.$objp->facid.'">'.img_object($langs->trans('ShowBill'),'bill').' ';
				print $objp->ref;
				print "</a></td>\n";
				// Ref supplier
