...
/**
 *   	\file       factures_depenses/paymentdep.php
 *		\ingroup    depenses module
 *		\brief      page to create a payement
...
if ($action == 'add_paiement')
{
	...
	if (! $error)
	{
		...
		{
			$db->commit();
			$loc = DOL_URL_ROOT.'/factures_depenses/paiement/fiche.php?id='.$paiement_id;
			Header('Location: '.$loc);
			exit;
		}
		else
...
$invoicechargesrstatic = new Depenses($db);
...
if ($action == 'create' || $action == 'add_paiement')
{
	...
	$sql.= ' f.rowid as depid, f.ref, f.amount, f.total_ttc as total';
	$sql .= ' FROM '.MAIN_DB_PREFIX.'societe as s, '.MAIN_DB_PREFIX.'depenses as f';
	...
	if ($resql)
	{
		...
		if ($num)
		{
			...
			print '<form name="addpaiement" action="paymentdep.php" method="post">';
			print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
			print '<input type="hidden" name="action" value="add_paiement">';
			print '<input type="hidden" name="facid" value="'.$facid.'">';
			print '<input type="hidden" name="facnumber" value="'.$obj->ref.'">';
			...
			*/
			$sql = 'SELECT d.rowid as facid,d.rowid as ref, d.ref as facnumber,d.total_ttc, d.dated as df';
			$sql .= ', sum(pf.amount) as am';
			$sql .= ' FROM '.MAIN_DB_PREFIX.'depenses as d';
			$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'paiementdepenses_facturedep as pf ON pf.fk_facturefourn = d.rowid';
			$sql .= ' WHERE d.fk_soc = '.$facture->fk_soc;
			$sql .= ' AND d.paye = 0';
		    //$sql .= ' AND d.fk_statut = 1';  // Statut=0 => non validee, Statut=2 => annulee
			$sql .= ' GROUP BY d.rowid, facnumber, d.total_ttc, d.dated';
			$resql = $db->query($sql);
			...
			if ($resql)
			{
				...
				if ($num > 0)
				{
					...
					while ($i < $num)
					{
						...
						$var=!$var;
						print '<tr '.$bc[$var].'>';
						print '<td><a href="fiche.php?id='.$objp->facid.'">'.img_object($langs->trans('ShowBill'),'bill').' '.$objp->ref;
						print '</a></td>';
...
if (! $_GET['action'] && ! $_POST['action'])
{
	...
	$sql.= ' f.rowid as facid, f.rowid as ref, f.ref as facnumber, f.amount,';
	$sql.= ' s.rowid as socid, s.nom,';
	$sql.= ' c.libelle as paiement_type, p.num_paiement,';
	$sql.= ' ba.rowid as bid, ba.label';
	if (!$user->rights->societe->client->voir) $sql .= ", sc.fk_soc, sc.fk_user ";
	$sql.= ' FROM '.MAIN_DB_PREFIX.'paiementdepenses AS p';
	if (!$user->rights->societe->client->voir) $sql .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
	$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'paiementdepenses_facturedep AS pf ON p.rowid=pf.fk_paiementfourn';
	$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'depenses AS f ON f.rowid=pf.fk_facturefourn ';
	...
	if ($resql)
	{
		...
		while ($i < min($num,$limit))
		{
			...
			// Ref payment
			print '<td nowrap="nowrap"><a href="'.DOL_URL_ROOT.'/factures_depenses/paiement/fiche.php?id='.$objp->pid.'">'.img_object($langs->trans('ShowPayment'),'payment').' '.$objp->pid.'</a></td>';
